...
async def upload_glyph_image(pid: str, character: str, file: UploadFile = File(...), db: AsyncSession = Depends(get_db)):
    ...
    if g:
        g.strokes = strokes; g.width_ratio = wr
    else:
        g = Glyph(profile_id=pid, character=character, strokes=strokes, width_ratio=wr); db.add(g)
    await db.flush()
...
@router.post("/{pid}/analyze-sample")
async def analyze_sample(pid: str, file: UploadFile = File(...), db: AsyncSession = Depends(get_db)):
